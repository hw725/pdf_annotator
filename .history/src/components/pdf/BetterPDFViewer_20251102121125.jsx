...
export default function BetterPDFViewer({
  ...
  }, [effectiveReferenceId, getSourceArrayBuffer, allHighlights]);
  ...
  }, [effectiveReferenceId, getSourceArrayBuffer, localFile, allHighlights]);
    ...
    >
      <div className="pdf-toolbar">
        ...
        <div className="toolbar-group">
          <button className="btn" onClick={() => goToPage(currentPage - 1)} disabled={!numPages || currentPage <= 1} title="이전 페이지">←</button>
          <input className="input-number" type="number" min={1} max={numPages || 1} value={currentPage} onChange={(e) => goToPage(e.target.value)} />
          <span className="muted">/ {numPages || "?"}</span>
          <button className="btn" onClick={() => goToPage(currentPage + 1)} disabled={!numPages || currentPage >= numPages} title="다음 페이지">→</button>
        </div>
        ...
        <div className="toolbar-group">
          ...
          <div className="btn-group" role="group" aria-label="view modes">
            <button className={`btn ${viewMode === "fit-width" ? "active" : ""}`} onClick={() => setViewMode("fit-width")} title="너비 맞춤">너비</button>
            <button className={`btn ${viewMode === "fit-page" ? "active" : ""}`} onClick={() => setViewMode("fit-page")} title="페이지 맞춤">페이지</button>
            <button className={`btn ${viewMode === "actual" ? "active" : ""}`} onClick={() => setViewMode("actual")} title="실제 크기(100%)">100%</button>
          </div>
        </div>
        ...
        <div className="toolbar-group">
          <PDFHighlight
            ...
            compact
          />
          <span className="muted">{Array.isArray(allHighlights) ? allHighlights.length : 0}개</span>
        </div>
        ...
        {/* Edge 호환 토글 */}
        <div className="toolbar-group">
          <label className="checkbox" title="Edge 렌더 호환 모드 (SVG)">
            <input type="checkbox" checked={compatMode} onChange={(e) => setCompatMode(e.target.checked)} />
            <span>Edge</span>
          </label>
        </div>
        ...
        {/* 업로드 + 동기화 + 내보내기 */}
        <div className="toolbar-group">
          <button className="btn primary" onClick={() => fileInputRef.current && fileInputRef.current.click()} title="로컬 PDF 업로드">업로드</button>
          {saving && <span className="muted">저장 중…</span>}
          {pendingCount > 0 && (
            <button className="btn warn" onClick={processSyncQueue} title="대기 중인 동기화 작업을 즉시 재시도">동기화 대기 <span className="badge">{pendingCount}</span></button>
          )}
          <div className="btn-split">
            <button className="btn info" onClick={handlePreviewExport} disabled={exportBusy} title="주석 포함 PDF 미리보기">미리</button>
            <button className="btn success" onClick={handleDownloadExport} disabled={exportBusy} title="주석 포함 PDF 다운로드">다운</button>
            <button className="btn" onClick={handleLocalSave} disabled={exportBusy} title="로컬 파일로 저장(브라우저 지원 시 대화상자)">로컬</button>
            <button className="btn drive" onClick={handleDriveSave} disabled={exportBusy} title={driveInfo.updatable ? "Drive에 주석 포함 업데이트" : "Drive에 주석 포함 저장"}>Drive</button>
          </div>
