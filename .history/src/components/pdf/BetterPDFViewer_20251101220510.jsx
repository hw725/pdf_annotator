...
export default function BetterPDFViewer({
  ...
  const getSourceArrayBuffer = useCallback(async () => {
    ...
    // { url: string }
    if (normalizedFile && typeof normalizedFile === "object" && "url" in normalizedFile) {
  ...
  const scrollToHighlight = useCallback(
    (h) => {
      ...
      const offsetWithinPage = baseH > 0 ? (rectY / baseH) * pagePixelHeight : 0;

      const target = currentTop + pageTopDelta + Math.max(0, offsetWithinPage - 80);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }} />
        {/* 호환 모드 토글 */}
        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#374151", marginRight: 8 }}>
          <input
            type="checkbox"
            checked={compatMode}
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: 8 }}>
            ...
            {exportBusy && (
              <span style={{ fontSize: 12, color: "#6b7280" }}>내보내는 중…</span>
            )}
          </div>
        </div>
      </div>
