...
export default function BetterPDFViewer({
          ...
          if (it.action === "save") {
            ...
            if (serverId && it.local_id) {
              setAllHighlights((prev) =>
                prev.map((h) => (h.id === it.local_id ? { ...h, remote_id: serverId } : h))
              );
            }
  ...
  // PDF 파일 소스 정규화
  const normalizedFile = typeof file === "string" ? { url: file, withCredentials: false } : file;
  ...
  const handleHighlightAdded = useCallback(
    async (highlight) => {
      ...
      onAnnotationChange &&
        onAnnotationChange((prev) => (prev ? [...prev, highlight] : [highlight]));
      ...
      try {
        ...
        const payload = {
          ...
          position:
            highlight.type === "text" ? { rects: highlight.rects } : { area: highlight.area },
        ...
        try {
          ...
          if (serverId) {
            setAllHighlights((prev) =>
              prev.map((h) => (h.id === highlight.id ? { ...h, remote_id: serverId } : h))
            );
          }
        } catch (e) {
          console.warn("주석 원격 저장 실패", e);
          await localDB.addToSyncQueue({ action: "save", payload, local_id: highlight.id });
          refreshPendingCount();
        } finally {
          setSaving(false);
        }
      ...
      try {
        ...
        if (targetId) {
          try {
            ...
            await localDB.addToSyncQueue({ action: "delete", target_id: targetId });
            refreshPendingCount();
          }
        }
      } catch (e) {
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <button onClick={() => goToPage(currentPage - 1)} disabled={!numPages || currentPage <= 1}>
            ←
          </button>
          <input
            type="number"
          ...
          />
          <span>/ {numPages || "?"}</span>
          <button onClick={() => goToPage(currentPage + 1)} disabled={!numPages || currentPage >= numPages}>
            →
          </button>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: 12 }}>
          <button onClick={zoomOut}>-</button>
          <span style={{ width: 48, textAlign: "center" }}>{Math.round(scale * 100)}%</span>
          <button onClick={zoomIn}>+</button>
          <button onClick={resetZoom} style={{ marginLeft: 6 }}>
            맞춤
          </button>
        </div>
        <div style={{ flex: 1 }} />
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: 12 }}>
          {saving && <div style={{ fontSize: 12, color: "#6b7280" }}>저장 중…</div>}
          {pendingCount > 0 && (
            <button
      ...
      {pdfError && <div style={{ padding: 12, color: "#dc2626", fontSize: 12 }}>{pdfError}</div>}
    </div>
  );
}
