...
export default function BetterPDFViewer({
  ...
  const fitToWidth = () => {
    const container = containerRef.current;
    const firstPage = pageRefs.current[1];
    if (!container || !firstPage) return;
    const pageCanvas = firstPage.querySelector(".react-pdf__Page__canvas");
    if (!pageCanvas) return;
    const containerWidth = container.clientWidth - 16; // padding 제외
    const pageWidth = pageCanvas.width / (window.devicePixelRatio || 1);
  ...
  const fitToHeight = () => {
    ...
    const pageCanvas = firstPage.querySelector(".react-pdf__Page__canvas");
    if (!pageCanvas) return;
    const containerHeight = container.clientHeight - 16; // padding 제외
    const pageHeight = pageCanvas.height / (window.devicePixelRatio || 1);
