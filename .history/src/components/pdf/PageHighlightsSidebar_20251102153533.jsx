...
export default function PageHighlightsSidebar({
  ...
  const byPage = useMemo(() => {
    // 문서 전체에서 동일한 내용(타입/색상/좌표/텍스트)이면 1개만 유지
    const makeDocSig = (h) => {
    ...
    (highlights || []).forEach((h) => {
      ...
      if (seenDoc.has(sig)) return; // 문서 전체 중복 제거(첫 번째 등장 페이지만 표시)
      seenDoc.add(sig);
  ...
  const flatList = useMemo(() => {
    ...
    src.sort((a, b) => (a.page || 0) - (b.page || 0) || (a.created_at || 0) - (b.created_at || 0));
    for (const h of src) {
      const sig = makeDocSig(h);
  ...
  return (
    ...
    >
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#6b7280" }}>
          ...
          <label style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }} title="페이지별 그룹 표시">
      ...
      <div>
        {byPage.map(({ page, list }) => {
          ...
          <div style={{ padding: "8px 8px 12px 8px", background: "#fff" }}>
            {flatList.map((h, idx) => (
              <button
                key={`flat-${idx}-${h.type}-${h.color}-${(h.text || "").slice(0,50)}`}
                onClick={() => onJumpToHighlight && onJumpToHighlight(h)}
                style={{
                  display: "flex",
                  width: "100%",
                ...
                title={h.text || "영역 하이라이트"}
              >
                <span style={{ minWidth: 36, color: "#6b7280" }}>p{h.page}</span>
                <span style={{ flex: 1 }}>
                  {h.type === "text"
                    ? (h.text || "텍스트 하이라이트").slice(0, 120)
                    : "영역 하이라이트"}
