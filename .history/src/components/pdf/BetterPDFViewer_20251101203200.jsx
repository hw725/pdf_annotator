  ...
  const handleError = (err) => {
    ...
    onLoadError && onLoadError(err);
        ...
        try {
          ...
          const serverId = resp?.annotation?.id;
          if (serverId) highlight.remote_id = serverId;
        } catch (e) {
  ...
  const zoomOut = () => setScale((s) => Math.max(0.5, s - 0.2));
      ...
      onAnnotationChange &&
        ...
        try {
          ...
          const serverId = resp?.annotation?.id;
          if (serverId) {
            // highlight 객체 자체를 돌려받은 참조로 보유하므로, 속성 추가로 원격 ID 보관
            highlight.remote_id = serverId;
          }
        } catch (e) {
          console.warn("주석 원격 저장 실패", e);
        }
  ...
  const handleHighlightDeleted = useCallback(
    async (highlight) => {
      try {
        ...
        if (targetId) {
          deleteAnnotation(targetId).catch((e) =>
