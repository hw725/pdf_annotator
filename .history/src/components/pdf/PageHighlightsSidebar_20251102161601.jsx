...
export default function PageHighlightsSidebar({
  ...
  const [hideDuplicates, setHideDuplicates] = useState(true); // 중복 숨김 토글(기본 ON)

  const byPage = useMemo(() => {
    // flatList(문서 전체 중복 제거 결과)에서 페이지별 그룹 생성
    const map = new Map();
    (flatList || []).forEach((h) => {
  ...
  const flatList = useMemo(() => {
    ...
    // 중복 숨김이 꺼진 경우, 원본 정렬만 적용
    return src;
  }, [highlights, hideDuplicates]);
  ...
  return (
    ...
    >
      <div style={{ padding: 12, borderBottom: "1px solid #e5e7eb" }}>
        ...
        >
          <span>총 {(flatList?.length ?? 0) || (highlights?.length ?? 0)}개</span>
          <label
            style={{
              marginLeft: "auto",
              display: "flex",
          ...
          >
            ...
            />
            <span style={{ color: "#374151" }}>중복 숨김</span>
          </label>
        </div>
      </div>
      ...
      <div>
        {groupByPage ? (
          ...
          <div style={{ padding: "8px 8px 12px 8px", background: "#fff" }}>
            {(flatList.length > 0 ? flatList : (Array.isArray(highlights) ? highlights : [])).map((h, idx) => (
              <button
            ...
            {flatList.length === 0 && (!highlights || highlights.length === 0) && (
