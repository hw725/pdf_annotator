...
export default function BetterPDFViewer({ file, referenceId, initialAnnotations = [], onAnnotationChange, onLoadSuccess, onLoadError }) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      Object.entries(pageRefs.current).forEach(([p, el]) => {
        ...
        if (d < min) { min = d; closest = parseInt(p, 10); }
      });
  ...
  const handleHighlightAdded = useCallback(async (highlight) => {
    try {
    ...
    onAnnotationChange && onAnnotationChange((prev) => (prev ? [...prev, highlight] : [highlight]));
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 12px", borderBottom: "1px solid #eee", background: "#fafafa" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span>/ {numPages || "?"}</span>
          <button onClick={() => goToPage(currentPage + 1)} disabled={!numPages || currentPage >= numPages}>→</button>
        </div>
          ...
          {normalizedFile ? (
            <Document file={normalizedFile} onLoadSuccess={handleLoad} onLoadError={handleError}>
              {numPages && Array.from({ length: numPages }, (_, i) => (
                <div key={`page_${i + 1}`} ref={(el) => (pageRefs.current[i + 1] = el)} style={{ position: "relative", marginBottom: 16 }}>
