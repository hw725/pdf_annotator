...
import { Document, Page, pdfjs } from "react-pdf";import { useState, useEffect, useRef, useCallback } from "react";import { useState, useEffect, useRef, useCallback } from "react";

import "react-pdf/dist/Page/AnnotationLayer.css";

import "react-pdf/dist/Page/TextLayer.css";import { Document, Page, pdfjs } from "react-pdf";import { Document, Page, pdfjs } from "react-pdf";

import PDFHighlight from "./PDFHighlight";

import PageHighlightOverlay from "./PageHighlightOverlay";import "react-pdf/dist/Page/AnnotationLayer.css";import "react-pdf/dist/Page/AnnotationLayer.css";
...
pdfjs.GlobalWorkerOptions.workerSrc = `//unpkg.com/pdfjs-dist@${pdfjs.version}/build/pdf.worker.min.mjs`;import PDFHighlight from "./PDFHighlight";import PDFHighlight from "./PDFHighlight";



const PDFViewer = ({import PageHighlightOverlay from "./PageHighlightOverlay";import PageHighlightOverlay from "./PageHighlightOverlay";

  file,

  referenceId,import { saveAnnotation, deleteAnnotation } from "../../api/refManagerClient";

  initialAnnotations = [],

  onAnnotationChange,// PDF.js worker 설정
  ...
  const [scale, setScale] = useState(1.0);const PDFViewer = ({

  const pdfType = null;

  const [highlightMode, setHighlightMode] = useState(null);const PDFViewer = ({  file,

  const [selectedColor, setSelectedColor] = useState("#FFFF00");

  const containerRef = useRef(null);  file,  referenceId,

  const pageRefs = useRef({});

  const [annotations, setAnnotations] = useState(initialAnnotations);  referenceId,  pdfCacheId,

  const [basePageWidth, setBasePageWidth] = useState(null);

  const [basePageHeight, setBasePageHeight] = useState(null);  initialAnnotations = [],  onLoadSuccess,

  const [viewMode, setViewMode] = useState("fit-width");

  const [saving, setSaving] = useState(false);  onAnnotationChange,  onLoadError,



  useEffect(() => {  onLoadSuccess,  onFilePicked, // 파일 선택(업로드) 콜백 (선택)

    setAnnotations(initialAnnotations);

  }, [initialAnnotations]);  onLoadError,}) => {
  ...
  const handleDocumentLoadSuccess = ({ numPages }) => {}) => {  const [numPages, setNumPages] = useState(null);
    ...
    if (onLoadSuccess) onLoadSuccess({ numPages });  const [numPages, setNumPages] = useState(null);  const [currentPage, setCurrentPage] = useState(1);
  ...
  const handleDocumentLoadError = (error) => {
    ...
    console.error("PDF 로드 에러:", error);  const [scale, setScale] = useState(1.0);  const pdfType = null; // 'text' or 'image' (향후 필요 시 감지 로직 연결)
  ...
  useEffect(() => {  const [highlightMode, setHighlightMode] = useState(null);  const [selectedColor, setSelectedColor] = useState("#FFFF00");

    const container = containerRef.current;

    if (!container) return;  const [selectedColor, setSelectedColor] = useState("#FFFF00");  const containerRef = useRef(null);



    const handleScroll = () => {  const containerRef = useRef(null);  const pageRefs = useRef({});

      let closestPage = 1;

      let minDistance = Infinity;  const pageRefs = useRef({});  const [sessionHighlights, setSessionHighlights] = useState([]); // 임시 업로드 세션 하이라이트



      Object.entries(pageRefs.current).forEach(([pageNum, element]) => {  const [annotations, setAnnotations] = useState(initialAnnotations);  const [driveInfo, setDriveInfo] = useState({

        if (element) {

          const rect = element.getBoundingClientRect();  const [basePageWidth, setBasePageWidth] = useState(null);    updatable: false,

          const containerRect = container.getBoundingClientRect();

          const pageCenter = rect.top - containerRect.top + rect.height / 2;  const [basePageHeight, setBasePageHeight] = useState(null);    fileId: null,

          const viewportCenter = container.clientHeight / 2;

          const distance = Math.abs(pageCenter - viewportCenter);  const [viewMode, setViewMode] = useState("fit-width");  });
          ...
          if (distance < minDistance) {  const [saving, setSaving] = useState(false);  const [basePageWidth, setBasePageWidth] = useState(null); // scale=1 기준 첫 페이지 폭(CSS px)
            ...
            closestPage = parseInt(pageNum);  const [basePageHeight, setBasePageHeight] = useState(null); // scale=1 기준 첫 페이지 높이(CSS px)
    ...
    container.addEventListener("scroll", handleScroll);  // 파일 변경 시 세션 하이라이트 초기화
  ...
  const goToPage = (pageNum) => {    setNumPages(numPages);    setSessionHighlights([]);

    if (pageNum >= 1 && pageNum <= numPages) {

      pageRefs.current[pageNum]?.scrollIntoView({ behavior: "smooth", block: "start" });    if (onLoadSuccess) onLoadSuccess({ numPages });  }, [file]);
  ...
  const zoomIn = () => {  // Drive 업데이트 가능 여부 감지 (캐시/Reference에서 drive_url 찾기)

    setViewMode("custom");

    setScale((prev) => Math.min(prev + 0.2, 3.0));  const handleDocumentLoadError = (error) => {  useEffect(() => {

  };

  const zoomOut = () => {    console.error("PDF 로드 에러:", error);    let cancelled = false;

    setViewMode("custom");

    setScale((prev) => Math.max(prev - 0.2, 0.5));    if (onLoadError) onLoadError(error);    (async () => {

  };

  const resetZoom = () => setViewMode("fit-width");  };      try {



  const computeScaleForMode = useCallback(        setDriveInfo({ updatable: false, fileId: null });

    (mode) => {

      if (!containerRef.current) return null;  useEffect(() => {        if (!referenceId && !pdfCacheId) return;

      const el = containerRef.current;

      if (mode === "fit-width" && basePageWidth) {    const container = containerRef.current;        const [

        return Math.max((el.clientWidth - 16) / basePageWidth, 0.1);

      }    if (!container) return;          { getCachedPDF, getCachedPDFByReferenceId },

      if (mode === "fit-page" && basePageHeight) {

        return Math.max((el.clientHeight - 16) / basePageHeight, 0.1);          { extractFileIdFromUrl },

      }

      if (mode === "actual") return 1.0;    const handleScroll = () => {        ] = await Promise.all([
  ...
  useEffect(() => {        ]);
    ...
    const s = computeScaleForMode(viewMode);      Object.entries(pageRefs.current).forEach(([pageNum, element]) => {        let cache = null;

    if (s && s > 0) setScale(s);

  }, [viewMode, basePageWidth, basePageHeight, computeScaleForMode]);        if (element) {        if (pdfCacheId) {



  useEffect(() => {          const rect = element.getBoundingClientRect();          cache = await getCachedPDF(pdfCacheId);

    const el = containerRef.current;

    if (!el) return;          const containerRect = container.getBoundingClientRect();        } else if (referenceId) {

    const ro = new ResizeObserver(() => {

      if (viewMode === "custom") return;          const pageCenter = rect.top - containerRect.top + rect.height / 2;          cache = await getCachedPDFByReferenceId(referenceId);

      const s = computeScaleForMode(viewMode);

      if (s && s > 0) setScale(s);          const viewportCenter = container.clientHeight / 2;        }

    });

    ro.observe(el);          const distance = Math.abs(pageCenter - viewportCenter);        const driveUrl = cache?.drive_url;

    return () => ro.disconnect();

  }, [viewMode, computeScaleForMode]);        if (driveUrl) {
