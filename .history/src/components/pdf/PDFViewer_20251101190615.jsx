/* eslint-disable react/prop-types *//* eslint-disable react/prop-types *//* eslint-disable react/prop-types *//* eslint-disable react/prop-types */
...
const PDFViewer = ({
  ...
  file,import PageHighlightOverlay from "./PageHighlightOverlay";import "react-pdf/dist/Page/AnnotationLayer.css";import "react-pdf/dist/Page/AnnotationLayer.css";
  ...
  const [selectedColor, setSelectedColor] = useState("#FFFF00");const PDFViewer = ({import PageHighlightOverlay from "./PageHighlightOverlay";import PageHighlightOverlay from "./PageHighlightOverlay";

  const containerRef = useRef(null);

  const pageRefs = useRef({});  file,

  const [annotations, setAnnotations] = useState(initialAnnotations);

  const [basePageWidth, setBasePageWidth] = useState(null);  referenceId,import { saveAnnotation, deleteAnnotation } from "../../api/refManagerClient";

  const [basePageHeight, setBasePageHeight] = useState(null);

  const [viewMode, setViewMode] = useState("fit-width");  initialAnnotations = [],
  ...
  const handleDocumentLoadError = (error) => {  const [currentPage, setCurrentPage] = useState(1);

    console.error("PDF 로드 에러:", error);

    if (onLoadError) onLoadError(error);  const [scale, setScale] = useState(1.0);const PDFViewer = ({
  ...
  useEffect(() => {
    ...
    const container = containerRef.current;  const [highlightMode, setHighlightMode] = useState(null);const PDFViewer = ({  file,
  ...
  const pageRefs = useRef({});
      ...
      Object.entries(pageRefs.current).forEach(([pageNum, element]) => {
        ...
        if (element) {  const [annotations, setAnnotations] = useState(initialAnnotations);  referenceId,  pdfCacheId,
          ...
          const viewportCenter = container.clientHeight / 2;  const [basePageHeight, setBasePageHeight] = useState(null);  initialAnnotations = [],  onLoadSuccess,
  ...
  const goToPage = (pageNum) => {
    ...
    if (pageNum >= 1 && pageNum <= numPages) {    setNumPages(numPages);
  ...
  const zoomIn = () => {
    ...
    setViewMode("custom");  const [currentPage, setCurrentPage] = useState(1);  const [scale, setScale] = useState(1.0);
  ...
  const zoomOut = () => {    console.error("PDF 로드 에러:", error);  const [scale, setScale] = useState(1.0);  const pdfType = null; // 'text' or 'image' (향후 필요 시 감지 로직 연결)

    setViewMode("custom");

    setScale((prev) => Math.max(prev - 0.2, 0.5));    if (onLoadError) onLoadError(error);
  ...
  const computeScaleForMode = useCallback(
    ...
    (mode) => {  useEffect(() => {  const [highlightMode, setHighlightMode] = useState(null);  const [selectedColor, setSelectedColor] = useState("#FFFF00");
      ...
      return null;      let closestPage = 1;

    },

    [basePageWidth, basePageHeight]      let minDistance = Infinity;  const pageRefs = useRef({});  const [sessionHighlights, setSessionHighlights] = useState([]); // 임시 업로드 세션 하이라이트
  ...
  useEffect(() => {
    ...
    if (viewMode === "custom") return;      Object.entries(pageRefs.current).forEach(([pageNum, element]) => {  const [annotations, setAnnotations] = useState(initialAnnotations);  const [driveInfo, setDriveInfo] = useState({

    const s = computeScaleForMode(viewMode);

    if (s && s > 0) setScale(s);        if (element) {
  ...
  useEffect(() => {
    ...
    const el = containerRef.current;          const containerRect = container.getBoundingClientRect();

    if (!el) return;

    const ro = new ResizeObserver(() => {          const pageCenter = rect.top - containerRect.top + rect.height / 2;  const [basePageHeight, setBasePageHeight] = useState(null);    fileId: null,

      if (viewMode === "custom") return;

      const s = computeScaleForMode(viewMode);          const viewportCenter = container.clientHeight / 2;
