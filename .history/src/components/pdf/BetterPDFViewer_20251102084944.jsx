    ...
    >
      <div className="pdf-toolbar">
        ...
        <div className="toolbar-group">
          ...
          {pendingCount > 0 && (
            <button className="btn warn" onClick={processSyncQueue} title="대기 중인 동기화 작업을 즉시 재시도">
              동기화 대기 <span className="badge">{pendingCount}</span>
            </button>
          )}

          <div className="btn-split">
            <button className="btn info" onClick={handlePreviewExport} disabled={exportBusy} title="주석 포함 PDF 미리보기">미리보기</button>
            <button className="btn success" onClick={handleDownloadExport} disabled={exportBusy} title="주석 포함 PDF 다운로드">다운로드</button>
            <button className="btn" onClick={handleLocalSave} disabled={exportBusy} title="로컬 파일로 저장(브라우저 지원 시 대화상자)">로컬 저장</button>
            <button className="btn drive" onClick={handleDriveSave} disabled={exportBusy} title={
                driveInfo.updatable
                  ? "Drive에 주석 포함 업데이트"
                  : "Drive에 주석 포함 저장"
              }>
