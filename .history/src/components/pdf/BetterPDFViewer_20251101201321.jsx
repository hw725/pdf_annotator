...
export default function BetterPDFViewer({
  ...
  const urlParams =
    typeof window !== "undefined"
      ...
      : null;
  const initialCompat =
    urlParams?.get("compat") === "1" || urlParams?.get("compat") === "true";
  const [compatOn, setCompatOn] = useState(!!initialCompat);
  const [prefetched, setPrefetched] = useState(false);
  const [fileSource, setFileSource] = useState(null);
  // (브라우저 UA 분기 제거) compatOn 상태로만 옵션을 제어합니다.
