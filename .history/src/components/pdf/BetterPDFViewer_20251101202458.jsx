...
export default function BetterPDFViewer({
  ...
  // 호환 모드 토글 (Edge 등 문제 시 수동 전환)
  const handleCompatToggle = useCallback(() => {
    const next = !compatOn;
    setCompatOn(next);
    setPdfError(null);
    setPrefetched(false);
    // URL 소스 기준으로 다시 로드 시작
    setFileSource(normalizedFile);
    // 주소창 파라미터 동기화 (새로고침 없이)
    try {
      if (typeof window !== "undefined") {
        const url = new URL(window.location.href);
        if (next) url.searchParams.set("compat", "1");
        else url.searchParams.delete("compat");
        window.history.replaceState({}, "", url.toString());
      }
    } catch (_) {}
  }, [compatOn, normalizedFile]);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        ...
        <div
          ...
          title="Edge 등에서 로딩 문제가 있을 때 켜세요"
        >
          <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <input
              type="checkbox"
              checked={compatOn}
              onChange={handleCompatToggle}
            />
            호환 모드
          </label>
        </div>
