...
export const updateDriveFile = async (fileId, fileBlob) => {
  ...
  const res = await fetch(
    ...
    { method: "PATCH", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/pdf" }, body: fileBlob }
  );
...
export const isDriveAPIAvailable = () => {
  const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;
  const apiKey = import.meta.env.VITE_GOOGLE_API_KEY;
  return !!(clientId && apiKey && clientId !== "your-client-id.apps.googleusercontent.com");
};
