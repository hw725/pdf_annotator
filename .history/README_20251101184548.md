...
이 앱은 Base44 RefManager와 분리되어 독립적으로 호스팅되는 PDF 주석 편집기입니다. RefManager의 라이브러리 제약사항을 우회하면서 완전한 PDF 주석 기능을 제공합니다.
...
`.env.local` 파일을 생성하고 RefManager API URL을 설정:
...
- `referenceId` (필수): RefManager 참고문헌 ID
- `token` (필수): Base44 인증 토큰
